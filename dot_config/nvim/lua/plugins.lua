return {
	...
	-- LSP setup with nvim-lspconfig
	{
		...
		config = function(_, opts)
			...
			-- Configure language servers using native vim.lsp.config (Neovim 0.11+)
			-- Get LSP capabilities from blink.cmp
			-- This is critical for diagnostics, completion, and other LSP features
			...
			local function toggle_eslint()
				...
				if eslint_enabled then
					-- Start eslint if not running
					if #clients == 0 then
						vim.cmd("LspStart eslint")
					end
					print("ESLint enabled")
				else
					-- Stop all eslint clients
			...
			vim.lsp.config("ocamllsp", {
				...
				root_markers = { "dune-project", "dune-workspace", ".opam", "*.opam", ".git", ".merlin" },
			})
